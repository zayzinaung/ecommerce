					...
					<tbody>
						...
						@foreach ($color as $row)
							<tr>
								...
								<td>
									@if($row->color_image != '')
										<img src="{{ URL::to('uploads/color_images/'.$row->color_image) }}" alt="{{ $row->color_name }}" title="{{ $row->color_name }}" width="65" height="35">
									@else
										<img src="{{ URL::to('uploads/no_icon.png') }}" alt="{{ $row->color_name }}" title="{{ $row->color_name }}" class="img-rounded" width="50">
									@endif
								</td>
								...
								<td>
									@if(User::hasPermTo(MODULE,'view'))
										<a href="{{ URL::to('admin/color/'.$row->id) }}" class="btn green-meadow tooltips" data-placement="bottom" data-original-title="View"><span aria-hidden="true" class="icon-eye"></span></a>
									@endif
									@if(User::hasPermTo(MODULE,'edit'))
										<a href="{{ URL::to('admin/color/'.$row->id.'/edit/') }}" class="btn blue-hoki tooltips" data-placement="bottom" data-original-title="Edit"><span aria-hidden="true" class="icon-pencil"></span></a>
									@endif
									@if(User::hasPermTo(MODULE,'delete'))
										{{ Form::open(array('method' => 'DELETE', 'route' => array('admin.color.destroy', $row->id), 'style'=>'display: inline')) }}
