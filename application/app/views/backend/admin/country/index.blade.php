					...
					<tbody>
						...
						@foreach ($country as $row)
							<tr>
								...
								<td>
									@if($row->flag != '')
										<img src="{{ URL::to('uploads/flags/'.$row->flag) }}" alt="{{ $row->country_name }}" title="{{ $row->country_name }}">
									@else
										<img src="{{ URL::to('uploads/no_icon.png') }}" alt="{{ $row->country_name }}" title="{{ $row->country_name }}" class="img-rounded" width="50">
									@endif
								</td>
								<td>{{ $row->calling_code }}</td>
								<td>
									@if(User::hasPermTo(MODULE,'view'))
										<a href="{{ URL::to('admin/country/'.$row->id) }}" class="btn green-meadow tooltips" data-placement="bottom" data-original-title="View"><span aria-hidden="true" class="icon-eye"></span></a>
									@endif
									@if(User::hasPermTo(MODULE,'edit'))
										<a href="{{ URL::to('admin/country/'.$row->id.'/edit/') }}" class="btn blue-hoki tooltips" data-placement="bottom" data-original-title="Edit"><span aria-hidden="true" class="icon-pencil"></span></a>
									@endif
									@if(User::hasPermTo(MODULE,'delete'))
										{{ Form::open(array('method' => 'DELETE', 'route' => array('admin.country.destroy', $row->id), 'style'=>'display: inline')) }}
