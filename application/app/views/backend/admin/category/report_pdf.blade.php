...
<body>
    <h3 style="margin-bottom:15px;text-align:center;text-decoration:underline;">Category Report</h3>
    <table class="collapse">
    <thead>
          <tr>
              <th style="width:1%">#</th>
              <th style="width:40%;text-align:left;">Category Name</th>
    ...
    <tr>
          ...
          <td class="vertical">{{ $category->category_name }}</td>
          <td class="vertical">{{ date($date_format, strtotime($category->created_at)) }}</td>
          <td class="vertical">{{ date($date_format, strtotime($category->updated_at)) }}</td>
    </tr>
    @endforeach
    </tbody>
