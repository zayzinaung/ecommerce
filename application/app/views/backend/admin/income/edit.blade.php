		...
		<div class="col-md-7">
			...
			<div class="portlet box blue-madison">
				...
				<div class="portlet-body form">
					...
					<!-- BEGIN FORM-->
					{{ Form::model($income, array('method' => 'PUT', 'route'=> array('admin.income.update', $income->id),  'class'=>'form-horizontal')) }}
					
						<div class="form-body">

							<h3 class="form-section">Other Incomes Information</h3>
							
							@if ($errors->has('name') || $errors->has('amount') || $errors->has('received_date') ) 
							...
							<div class="form-group">
								<label class="control-label col-md-3">Other Incomes Name<span class="required">* </span></label>
								<div class="col-md-6">
									<select class="form-control" name="income_name" id="income_select">
										@if ( $income != null )
											<?php foreach ( $get_income as $e ) { $default = '' ?>
						                            				<?php if ( $income->id == $e->id ) { $default = 'selected'; } ?>
						                            				<option value="{{ $e->id }}" {{ $default }}>{{ $e->income_name }}</option>
						                            			<?php } ?>
						                            		@else
						                            			<?php foreach ( $get_income as $e ) { ?>
									                            <option value="{{ $e->id }}">{{ $e->income_name }}</option>
									                     <?php } ?>
										@endif
										<option value="0">Add Other Incomes</option>
									</select>
								</div>
							</div>
							<div class="form-group @if ($errors->has('name')) has-error @endif">
								...
								<div class="col-md-6">
									{{ Form::text('name', $income->income_name,array('class'=>'form-control income_data','readonly'=>'readonly')) }}
							...
							<div class="form-group @if ($errors->has('amount')) has-error @endif">
								<label class="control-label col-md-3">Other Incomes Amount<span class="required">* </span></label>
								<div class="col-md-6">
									{{ Form::text('amount', $income->income_amount,array('class'=>'form-control')) }}
							...
							<div class="form-group @if ($errors->has('received_date')) has-error @endif">
								<label class="control-label col-md-3">Payment Date<span class="required">* </span></label>
								<div class="col-md-6">
									{{ Form::text('received_date', date($date_format, $income->received_date),array('class'=>'form-control datepicker')) }}
									@foreach($errors->get('received_date') as $error)
									    <span class="help-inline"> {{ $error }}</span>
									@endforeach
								</div>
							</div>
							                    ...
							                    <div class="tab-content">
							                            <div class="tab-pane fade in pay_content1" id="cash">
							                              <p>The payment has been made by Cash...</p>
							                            	...
							                            	<?php 
							                            		$cheque = unserialize($income->cheque); 
								                            	if ( $cheque )
								                            	{
							                            ...
							                            <div class="tab-pane fade in pay_content2" id="cheque">
							                            	<label for="checque">
							                            		<input type="checkbox" name="cheque" value="1" class="enable" id="check2">
							                            ...
							                            <div class="tab-pane fade in pay_content3" id="enet">
							                            	<label for="checque">
							                              	...
							                              	<div class="control-group @if ($errors->has('transaction_id')) has-error @endif">
							                                	{{ Form::text('transaction_id', $income->enet, array('class'=>'form-control','placeholder'=>'Transaction ID')) }}
							                             ...
							                             <div class="tab-pane fade in pay_content4" id="other">
							                             	<label for="checque">
							                              	...
							                              	<div class="control-group @if ($errors->has('description')) has-error @endif">
							                                	{{ Form::textarea('description', $income->other, ['class' => 'form-control','id'=>'description','rows'=>'5']) }}
...
$(document).ready(function(){
	$('#income_select').change(function(){
		...
		$.post("{{ URL::to('admin/income/get_income') }}", param, function (data) {
		      	if (data.status == 'success') {

		      		if ( data.income != '' )
		      		{
		      			$('input[name=name]').val(data.income);
			          		$(".income_data").attr("readonly", "readonly");
			          	} else {
			          		$('input[name=name]').val('');
			          		$(".income_data").removeAttr("readonly");
			          	}
